...
export class WhatsAppAdapter implements TransportAdapter {
  ...
  ensureRecipient(phone: string): string {
    return phone.replace(/\D/g, ""); // E.164 solo dígitos
  }
  ...
  normalizeInbound(payload: unknown): InternalMessage[] {
    ...
    for (const change of changes) {
      ...
      // Ecos: mensajes que el negocio mandó desde afuera del CRM.
  ...
  private async post(payload: unknown): Promise<{ id: string }> {
    ...
    if (!res.ok) {
      // Nunca tragarse el fallo: el sistema debe saber que NO salió.
      // 131047 = ventana de 24 h cerrada -> el llamador reintenta con plantilla.
      const err = new Error(`WhatsApp send falló ${res.status}: ${JSON.stringify(data)}`) as Error & { status: number; code?: number };
  ...
  async fetchMedia(mediaId: string): Promise<{ bytes: Buffer; mimeType: string }> {
    ...
    if (!metaRes.ok) throw new Error(`fetchMedia meta falló ${metaRes.status}`);
    ...
    if (!bin.ok) throw new Error(`fetchMedia bytes falló ${bin.status}`);
