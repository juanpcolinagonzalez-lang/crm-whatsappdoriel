...
export type TiendanubeOrder = {
  id: number;
  number?: number; // numero de pedido correlativo que ve el cliente. NUNCA es el id interno.
...
/**
 * Numero de pedido que el cliente conoce (correlativo de la tienda, el que
 * aparece en el panel de Tiendanube y en los mails). NUNCA usar el id interno
 * (order.id) para comunicarle algo al cliente: son cosas distintas.
 */
export function orderNumber(o: Pick<TiendanubeOrder, "id" | "number">): string {
  return o.number != null ? String(o.number) : String(o.id);
}
...
/**
 * Trae productos publicados EN VIVO y devuelve solo los que tienen un
 * promotional_price activo (menor al precio de lista). Es la fuente real de
 * "que promos hay" cuando el cliente pregunta en general, sin nombrar un
 * producto puntual.
 */
export async function fetchPromotions(storeId: string, token: string) {
  const productos = await apiGet<TiendanubeProduct[]>(storeId, token, `/products?per_page=50&published=true`);
  return productos.filter((p) => {
    const v = p.variants?.[0];
    const precio = Number(v?.price ?? 0);
    const promo = Number(v?.promotional_price ?? 0);
    return promo > 0 && precio > 0 && promo < precio;
  });
}
