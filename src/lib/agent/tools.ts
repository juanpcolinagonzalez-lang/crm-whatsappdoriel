...
import { getOrderStatusLabel, searchProducts, fetchPromotions, productName, productImage, type TiendanubeProduct } from "@/lib/ecommerce/tiendanube";
...
export function makeTools(ctx: ToolCtx) {
  return {
    consultar_producto: tool({
      description:
        "Consulta un producto en la base de conocimiento propia del negocio (fuente de verdad de que existe). Usar SIEMPRE antes de informar nombre, descripcion, colores, link o precio; nunca de memoria. Si hay una tienda online conectada, complementa con precio y stock EN VIVO cuando encuentra el mismo producto alli; si no lo encuentra alli, el precio de la base sigue valiendo pero el stock queda sin verificar (no lo asegures, ofrece confirmarlo). Si el producto no esta en la base de conocimiento, NO EXISTE para el agente aunque pueda existir en la tienda: no lo ofrezcas. Tambien sirve para venta cruzada: si el cliente ya eligio algo, se puede volver a llamar con un producto complementario y ofrecerlo solo si aparece de verdad en la base.",
      parameters: z.object({
        consulta: z.string().describe("nombre o descripcion del producto que pide el cliente, o del complementario a sugerir"),
      }),
      execute: async ({ consulta }) => {
        const { data: productosBase } = await ctx.db
          .from("products")
          .select("id, name, description, colors, link, price, product_lines(name)")
          .eq("organization_id", ctx.orgId)
          .or(`name.ilike.%${consulta}%,description.ilike.%${consulta}%`)
          .limit(3);

        if (!productosBase || productosBase.length === 0) {
          return { encontrado: false, motivo: "no se encontro ese producto en la base de conocimiento del negocio" };
        }

        let liveProductos: TiendanubeProduct[] = [];
        ...
        if (conn) {
          try {
            ...
            liveProductos = await searchProducts(conn.store_id, token, consulta);
          } catch {
            liveProductos = [];
          }
        }
        ...
        const resultados = productosBase.map((p: any) => {
          const match = liveProductos.find((lp) => {
            const nombreLive = normalizar(productName(lp));
            const nombreBase = normalizar(p.name);
            return nombreLive.includes(nombreBase) || nombreBase.includes(nombreLive);
          });
          const variante = match?.variants?.[0];
          return {
            nombre: p.name,
            descripcion: p.description,
            colores: p.colors,
            link: p.link,
            linea: p.product_lines?.name ?? null,
            precio: variante?.promotional_price || variante?.price || p.price || null,
            stock: variante?.stock ?? null,
            stock_verificado: Boolean(match),
            imagen: match ? productImage(match) : null,
          };
        });
    ...
    estado_pedido: tool({
      description:
        "Consulta el estado de un pedido EN VIVO: estado general, si el pago esta confirmado o todavia pendiente de revision, si la entrega es por envio o por retiro en el local (con horario y direccion reales), y el link real de seguimiento del envio (tracking_url) si el transportista ya lo genero. Si no devuelve nada, NUNCA digas que el pedido no existe: tranquiliza al cliente (el equipo lo confirma, el seguimiento llega por mail). NUNCA inventes un link de checkout o de seguimiento: solo compartis el tracking_url si esta herramienta te lo devuelve.",
      parameters: z.object({
        referencia: z.string().optional().describe("numero de pedido, solo si el contacto tiene mas de un pedido y hace falta desambiguar"),
      }),
      execute: async ({ referencia }) => {
        ...
        const trackingUrl = raw.shipping_tracking_url ? String(raw.shipping_tracking_url) : undefined;
        const trackingNumber = raw.shipping_tracking_number ? String(raw.shipping_tracking_number) : undefined;
        const trackingCarrier = raw.shipping_carrier_name ? String(raw.shipping_carrier_name) : undefined;
...
import type { SupabaseClient } from "@supabase/supabase-js";
import { advanceLeadToRole } from "@/lib/pipeline/stage";
import { getOrderStatusLabel, searchProducts, fetchPromotions, productName, productImage } from "@/lib/ecommerce/tiendanube";
import { decrypt } from "@/lib/crypto";
import { getTransport } from "@/lib/transport";
