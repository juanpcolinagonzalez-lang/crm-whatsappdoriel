...
export async function processInbound(db: SupabaseClient, orgId: string, msg: InternalMessage): Promise<void> {
  ...
  const { data: pastOrders } = await db
    .from("orders")
    .select("external_id, status_raw, total, created_at, data")
    .eq("organization_id", orgId)
  ...
  // (data.number), NUNCA el id interno de Tiendanube (external_id).
  const lastOrderNumber = pastOrders?.[0]
    ? (pastOrders[0].data as Record<string, unknown> | null)?.number
      ? String((pastOrders[0].data as Record<string, unknown>).number)
      : pastOrders[0].external_id
    : null;

  const purchaseHistory = pastOrders?.length
    ? `Ya compró ${pastOrders.length} ${pastOrders.length === 1 ? "vez" : "veces"} antes. Pedido más reciente: #${lastOrderNumber}, estado ${getOrderStatusLabel(pastOrders[0].status_raw)}${pastOrders[0].total ? `, total $${pastOrders[0].total}` : ""}.`
    : null;
