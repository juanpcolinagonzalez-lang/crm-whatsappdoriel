        ...
        <>
              ...
              <ul className="flex-1 overflow-y-auto divide-y divide-slate-100">
                ...
                {conversations.length > 0 && filtered.length === 0 && (
                    <li className="p-6 text-sm text-slate-400">Sin resultados para esa búsqueda.</li>li>
                      )}
                {filtered.map((c) => {
                                  ...
                                  <li key={c.id}>
                                                <Link
                                                                ...
                                                                <div className="flex items-center justify-between gap-2">
                                                                                  ...
                                                                                  <span className="text-xs text-slate-400 shrink-0">{c.time}</span>span>
                                                                </div>div>
                                                                <div className="mt-0.5 flex items-center gap-2">
                                                                                  <span className="text-xs text-slate-500 truncate">{c.preview || "\u2014"}</span>span>
                                                                  {c.isPaused && (
